...
CHART_COLOR_PALETTE = {'Vivo': '#1E8449', 'Eutanasiado/Recolhido': '#C00000', 'Não especificado': '#808080'}
...
if df_ictio_periodo.empty:
    ...
    with col_graf1, st.container(border=True):
        st.subheader("Quantitade de peixes")
    ...
    with st.container(border=True):
        ...
        if tipo_analise == "Dia Específico":
            df_historico_filtrado = df_ictio_master[df_ictio_master['Resgate'].isin(fase_selecionada)]
            df_temporal = df_historico_filtrado.groupby(df_historico_filtrado['Data'].dt.date)['N°_Individuos'].sum().reset_index()
            df_temporal['Cor'] = 'Histórico'
            df_temporal.loc[df_temporal['Data'] == start_date, 'Cor'] = 'Dia Selecionado'
            palette = {'Histórico': '#006400', 'Dia Selecionado': '#FFC300'}
            fig_temporal = px.bar(df_temporal, x='Data', y='N°_Individuos', title="<b>Histórico Completo (Dia Selecionado em Destaque)</b>", color='Cor', color_discrete_map=palette)
        else:
            df_temporal = df_ictio_periodo.groupby(df_ictio_periodo['Data'].dt.date)['N°_Individuos'].sum().reset_index()
            fig_temporal = px.bar(df_temporal, x='Data', y='N°_Individuos', title="<b>Total de Indivíduos por Dia no Período</b>", color_discrete_sequence=['#006400'])
        
        fig_temporal.update_layout(title_x=0.5, height=400, legend_title_text='')
    ...
    with st.container(border=True):
        ...
        if not df_mapa.empty:
            ...
            fig_mapa = px.scatter_mapbox(df_mapa, lat="Latitude_num", lon="Longitude_num", size="N°_Individuos", color="Ponto_Amostral", hover_name="Ponto_Amostral",
                hover_data={"N°_Individuos": True, "Latitude_num": False, "Longitude_num": False},
                mapbox_style="satellite-streets", center=dict(lat=center_lat, lon=center_lon), zoom=15)
