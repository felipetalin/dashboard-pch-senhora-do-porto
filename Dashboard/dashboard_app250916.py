...
<style>
    ...
    /* LINHA DIVISÓRIA VERDE PARA O MODO RELATÓRIO */
    .green-divider {
        border-top: 2px solid #A9D9A9;
        margin-top: 20px;
        margin-bottom: 20px;
    }
</style>
...
if report_mode:
    ...
    _ , report_col, _ = st.columns([1, 4, 1]) # Margens de 1/6 e conteúdo de 4/6 (2/3)

    with report_col:
        if start_date == end_date:
            st.markdown(f"<div class='section-header'>Relatório do Dia: {start_date.strftime('%d/%m/%Y')}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div class='section-header'>Relatório do Período: {start_date.strftime('%d/%m/%Y')} a {end_date.strftime('%d/%m/%Y')}</div>", unsafe_allow_html=True)

        if df_ictio_periodo.empty:
            st.warning("Nenhuma atividade registrada para este período.")
        else:
            # --- KPIs ---
            ...
            kpi_col1.metric("Total Indivíduos", total_periodo)
            kpi_col2.metric("Total Vivos", vivos_periodo)
            kpi_col3.metric("Sobrev. (Nativos)", f"{taxa_sobrevivencia_nativos:.1f}%")
            ...
            fig_dist = px.bar(df_dist, x='Distribuição', y='N°_Individuos', color='Destino', text_auto=True, color_discrete_map=CHART_COLOR_PALETTE)
            fig_dist.update_layout(title_x=0.5, height=300)
            ...
            if not df_top10.empty:
                ...
                fig_top10.update_layout(title_x=0.5, height=300)
            ...
            fig_temporal = px.bar(df_temporal, x='Data', y='N°_Individuos', title="<b>Total de Indivíduos por Dia</b>", color_discrete_sequence=['#006400'])
            fig_temporal.update_layout(title_x=0.5, height=300)
    ...
    if df_ictio_periodo.empty:
        ...
        with st.container(border=True):
            st.subheader("Resgates ao Longo do Tempo")
            df_temporal = df_ictio_periodo.groupby(df_ictio_periodo['Data'].dt.date)['N°_Individuos'].sum().reset_index()
            # Gráfico de barras para o histórico, conforme solicitado
            fig_temporal = px.bar(df_temporal, x='Data', y='N°_Individuos', title="<b>Total de Indivíduos por Dia</b>", color_discrete_sequence=['#006400'])
            fig_temporal.update_layout(title_x=0.5, height=400)
            st.plotly_chart(fig_temporal, use_container_width=True)

        # Seção de fotos agora também aparece para o modo período
        if tipo_analise == "Período":
             with st.container(border=True):
                ...
                if not imagens_encontradas:
                    ...
                    cols_fotos = st.columns(2) # Mostra até 2 fotos
                    if len(imagens_encontradas) > 0:
                        with cols_fotos[0]: st.image(str(imagens_encontradas[0]), use_container_width=True)
                    if len(imagens_encontradas) > 1:
                        with cols_fotos[1]: st.image(str(imagens_encontradas[1]), use_container_width=True)
